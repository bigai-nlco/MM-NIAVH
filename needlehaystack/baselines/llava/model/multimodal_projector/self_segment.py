...
def segment(features, alpha=0.5, k=None):
    ...
    if k is not None:
        ...
        if len(boundaries) > 7: #TODO: limit max segments to prevent from OOM: 7 comes from RMT paper
            boundaries = torch.topk(depth_scores, 7).indices.sort()[0]
