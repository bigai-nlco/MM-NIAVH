...
class TransformerProjector(nn.Module):
    ...
    def forward(
        ...
        if output_attentions:
            return proj_hidden_states, read_memories, hidden_states, all_self_attentions, all_cross_attentions
        else:
...
class RMTRTransformerProjector(nn.Module):
    ...
    def forward(
        ...
        # hidden_states = torch.cat([cls_states.unsqueeze(1), hidden_state], dim=1)
        ...
        if t <= 3:
            ...
            # # last_hidden_states = image_output[0] # patch
            # last_hidden_states = image_output[1] # memory tokens
            # last_hidden_states = self.projector.proj(last_hidden_states)
            
            read_memories = image_output[1]
            read_memories = self.retrieval(
                read_memories,
                encoder_hidden_states = image_output[2]
            )
            last_hidden_states = self.projector.proj(read_memories)
            ...
            for s, bi in enumerate(boundaries):
                ...
                memory_cache.append(segment_output[2])
            ...
            # enc-dec
            enc_memories = torch.cat(enc_memories, dim=1)
            # last_hidden_states = torch.cat([enc_memories, segment_output[2]], dim=1)
            last_hidden_states = enc_memories
            last_hidden_states = self.projector.proj(last_hidden_states)
            # last_hidden_states = torch.cat([read_memories, segment_output[2]], dim=1)
            # last_hidden_states = self.projector.proj(last_hidden_states)
