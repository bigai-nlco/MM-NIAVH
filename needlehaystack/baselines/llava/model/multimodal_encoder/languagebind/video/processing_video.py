...
def load_and_transform_video(
    ...
    if video_decode_backend == 'pytorchvideo':
        ...
        if fps:
            ...
            # num_frames = max(8, duration - duration%8)
            num_frames = max(new_duration, 8)
            if num_frames % 8:
                num_frames = num_frames + 8 - num_frames % 8
            # print(num_frames)
        
        frame_id_list = np.linspace(0, duration-1, num_frames, dtype=int)
        video_data = decord_vr.get_batch(frame_id_list)
        
        # # debug the selected frames
        # frames_save_path = "debug_cache"
        # os.makedirs(frames_save_path, exist_ok=True)
        # for frame_idx, frame in zip(frame_id_list, video_data):
        #     image_path = os.path.join(frames_save_path, f"frame_{frame_idx:04d}.jpg")
        #     cv2.imwrite(image_path, cv2.cvtColor(frame.cpu().numpy(), cv2.COLOR_RGB2BGR))
                ...
                if fps:
                    if video.endswith(".gif"):
                        image_features.append(self.image_processor(video, self.transform,
                                                                   video_decode_backend="gif",fps=fps))
                    else:
                        image_features.append(self.image_processor(video, self.transform,
                                                                   video_decode_backend=self.config.vision_config.video_decode_backend, fps=fps))
                else:
                    if video.endswith(".gif"):
                        image_features.append(self.image_processor(video, self.transform, 
                        ...
                        image_features.append(self.image_processor(video, self.transform,
                                                    ...
                                                    num_frames=self.config.vision_config.num_frames)) # iteratively preprocess image
